...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['batch']));
...
foreach (array_filter((['batch']), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<article class="group relative rounded-2xl p-[1px] bg-gradient-to-br from-indigo-100 via-sky-100 to-violet-100 hover:from-indigo-300 hover:to-sky-300 transition-all duration-300 h-full">
    <div class="relative h-full rounded-2xl border border-white/70 bg-white/85 backdrop-blur-sm overflow-hidden shadow-[0_10px_28px_rgba(15,23,42,0.08)] group-hover:shadow-[0_18px_35px_rgba(79,70,229,0.22)] group-hover:-translate-y-1 transition-all duration-300 flex flex-col">
        ...
        <div class="p-5 flex flex-col flex-1">
            <h3 class="text-base font-black text-slate-900 leading-tight mb-2 line-clamp-2"><?php echo e($title); ?></h3>
            <p class="text-xs text-slate-500 leading-relaxed line-clamp-2 mb-4"><?php echo e($description); ?></p>
            ...
            <div class="mt-auto pt-4 border-t border-slate-100 flex items-end justify-between gap-3">
                <div>
                    <p class="text-[11px] text-slate-400 font-semibold line-through"><?php echo e(($mrp > 0) ? '₹' . number_format($mrp) : ''); ?></p>
                    <p class="text-2xl font-black text-slate-900 leading-none"><?php echo e($price > 0 ? '₹' . number_format($price) : 'Free'); ?></p>
                </div>
...
<?php /**PATH D:\Lama Projects\Toppershope\laravel_core\resources\views/components/batch-card.blade.php ENDPATH**/ ?>