...
<div class="flex items-center gap-2 text-sm text-slate-500 mb-6">
    <a href="{{ route('student.my-courses') }}" class="hover:text-orange-600 transition-colors">My Courses</a>
    <span>›</span>
                ...
                @foreach($course->subjects as $subject)
                    <div class="border-b border-slate-800/50 last:border-b-0">
                        <div class="px-4 py-3 bg-slate-100/30 flex justify-between items-center cursor-pointer select-none hover:bg-slate-50 transition-colors" onclick="toggleAccordion('subject-{{ $subject->id }}')">
                            <h4 class="text-xs font-bold text-orange-600 uppercase tracking-wide">{{ $subject->name }}</h4>
            ...
            @foreach($subject->chapters as $chapter)
                @foreach($chapter->units as $unit)
                    <div id="content-unit-{{ $unit->id }}" class="unit-content hidden transition-all duration-300">
                        <div class="mb-6">
                            <div class="flex items-center gap-2 text-[10px] font-bold text-orange-600 uppercase tracking-widest mb-2">
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-5 mb-4 border border-indigo-500/10 shadow-lg shadow-indigo-500/5">
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                @foreach($unit->videos as $video)
                                    <a href="{{ route('student.my-courses.video', [$enrollment->id, $video->id]) }}" class="flex items-center gap-4 p-4 rounded-xl bg-sky-500/5 border border-sky-500/20 hover:border-sky-500/50 transition-all group hover:bg-sky-500/10">
                                        <div class="w-10 h-10 rounded-xl bg-sky-500/10 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                                ...
                                @foreach($unit->notes as $note)
                                    <a href="{{ route('student.my-courses.note', [$enrollment->id, $note->id]) }}" class="flex items-center gap-4 p-4 rounded-xl bg-emerald-500/5 border border-emerald-200 hover:border-emerald-500/50 transition-all group hover:bg-emerald-500/10">
                                        ...
                                        <div class="flex-1 min-w-0 flex items-center justify-between">
                                            ...
                                            <div class="text-xs font-bold text-emerald-400/90 bg-emerald-500/10 px-4 py-2 rounded-lg border border-emerald-200">
                                                View PDF
                                            </div>
                                        </div>
                                    </a>
