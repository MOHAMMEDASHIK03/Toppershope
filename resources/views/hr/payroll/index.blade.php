    ...
    <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
        <form action="{{ route('hr.payroll.index') }}" method="GET" class="flex gap-2 flex-1 md:flex-none min-w-[10rem]">
            <input type="month" name="month_year_picker" value="{{ \Carbon\Carbon::createFromFormat('m-Y', $monthYear)->format('Y-m') }}"
                   ...
                   class="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary-600/15 focus:border-primary-500 outline-none font-medium text-slate-700 text-sm shadow-sm">
        ...
        <a href="{{ route('hr.payroll.create') }}" class="px-4 py-2.5 btn-primary font-semibold rounded-xl text-sm shadow-sm hover:bg-primary-700 focus:ring-4 focus:ring-primary-100 transition-colors inline-flex items-center gap-2 shrink-0">
    ...
    <div class="panel-list panel-list--payroll">
        ...
        @forelse($payrolls as $pr)
            <article class="panel-list__row">
                ...
                <div class="panel-list__cell--status">
                    ...
                    @if($pr->status === 'paid')
                        <span class="panel-list-status-pill bg-primary-50 border border-emerald-100 text-emerald-700">
                            <span class="w-1.5 h-1.5 rounded-full bg-primary-500 shrink-0"></span> Paid
                        </span>
                    @else
        ...
        @empty
            <div class="px-6 py-12 text-center text-slate-500 font-medium text-sm">
                ...
                <a href="{{ route('hr.payroll.create') }}" class="text-primary-700 hover:underline mt-2 inline-block">Generate payrolls now</a>
            </div>
        @endforelse
    </div>
