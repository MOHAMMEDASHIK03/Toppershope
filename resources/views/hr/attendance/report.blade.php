...
<div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
    ...
    <div class="flex items-center gap-3">
        <form action="{{ route('hr.attendance.report', $employee->id) }}" method="GET" class="flex gap-2">
            <input type="month" name="month" value="{{ $month }}" class="px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 outline-none font-medium text-slate-700 text-sm shadow-sm" onchange="this.form.submit()">
        </form>
        <a href="{{ request()->fullUrlWithQuery(['export' => 'csv']) }}" class="px-4 py-2.5 bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold rounded-xl text-sm shadow-sm hover:bg-emerald-100 transition-colors inline-flex items-center gap-2">
            ...
            <tr class="hover:bg-slate-50/50 transition-colors">
                ...
                <td class="px-6 py-4">
                    @if($record->status === 'present')
                        <span class="inline-flex items-center px-2.5 py-1 rounded-md bg-emerald-50 text-emerald-700 font-bold text-xs uppercase tracking-wider">Present</span>
                    ...
                    @elseif($record->status === 'half_day')
                        <span class="inline-flex items-center px-2.5 py-1 rounded-md bg-orange-50 text-blue-700 font-bold text-xs uppercase tracking-wider">Half Day</span>
                    @else
                        <span class="inline-flex items-center px-2.5 py-1 rounded-md bg-orange-50 text-indigo-700 font-bold text-xs uppercase tracking-wider">On Leave</span>
                    @endif
                </td>
