    ...
    <nav class="fixed w-full z-50 nav-shell transition-all duration-300" x-data="{ scrolled: false, mobileMenuOpen: false, coursesMenuOpen: false }" @scroll.window="scrolled = (window.pageYOffset > 10)">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-[78px]">
                                ...
                                <div class="max-w-7xl mx-auto px-6 py-8">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-6">
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            <a href="{{ route('category.show', $navCat) }}" class="flex items-center gap-3 p-2.5 rounded-xl hover:bg-blue-50 group/item transition-colors">
                                                <div class="w-9 h-9 rounded-lg bg-indigo-50 text-primary flex items-center justify-center shrink-0">
                                                    ...
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $navCat->landing('icon_svg') }}"></path></svg>
                                                    @endif
                                                </div>
                                                <div>
                                                    <p class="text-sm font-bold text-gray-900 group-hover/item:text-primary transition-colors">{{ $navCat->name }}</p>
                                                    <p class="text-xs text-gray-400">{{ $navCat->activeSubcategories->pluck('name')->take(3)->join(', ') }}</p>
                                                </div>
                                            </a>
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="border-t border-gray-100 pt-4 flex items-center justify-between">
                                        ...
                                        <a href="{{ url('/') }}" class="text-sm font-bold text-primary hover:text-blue-700 flex items-center gap-1 transition-colors">View All Courses &rarr;</a>
                                    </div>
                                </div>
                            </div>
                        
                        <!-- Simple Links -->
                        <a href="{{ route('about') }}" class="nav-chip text-sm font-semibold {{ request()->routeIs('about') ? 'text-primary bg-indigo-50' : 'text-gray-700' }} transition-colors">About Us</a>
                        <a href="{{ route('careers') }}" class="nav-chip text-sm font-semibold {{ request()->routeIs('careers*') ? 'text-primary bg-indigo-50' : 'text-gray-700' }} transition-colors">Careers</a>
                        <a href="{{ route('faq') }}" class="nav-chip text-sm font-semibold {{ request()->routeIs('faq') ? 'text-primary bg-indigo-50' : 'text-gray-700' }} transition-colors">FAQ</a>
                        <a href="{{ route('contact') }}" class="nav-chip text-sm font-semibold {{ request()->routeIs('contact') ? 'text-primary bg-indigo-50' : 'text-gray-700' }} transition-colors">Contact Us</a>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 lg:flex-1 shrink-0">
                    ...
                    @auth
                        <a href="{{ route('logout') }}" class="inline-block bg-white hover:bg-slate-100 text-gray-900 px-5 py-2.5 rounded-xl text-sm font-bold transition-all border border-slate-200">
                            Logout
                        </a>
                        <a href="{{ route('dashboard') }}" class="nav-cta-primary text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all">
                            Dashboard
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="nav-cta-primary text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-all">
                            Login / Register
                        </a>
                    @endauth
                </div>
        ...
        <div x-show="mobileMenuOpen" x-transition.opacity style="display: none;" @click.self="mobileMenuOpen = false" class="md:hidden fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-sm pt-[78px]">
            ...
            <div class="px-4 pt-4 pb-8 space-y-2" x-data="{ mobileCoursesOpen: false }">
                <a href="{{ url('/') }}" class="block px-3 py-2.5 rounded-xl font-semibold text-gray-900 hover:bg-gray-50 hover:text-primary">Home</a>
                ...
                <div>
                    ...
                    <div x-show="mobileCoursesOpen" x-transition.opacity style="display: none;" class="bg-gradient-to-b from-indigo-50/70 to-white rounded-2xl p-4 mt-2 mb-2 space-y-2 border border-indigo-100 max-h-[60vh] overflow-y-auto">
                        ...
                        <a href="{{ route('category.show', $navCat) }}" class="flex items-center gap-3 p-2 rounded-xl active:bg-blue-100 transition-colors">
                            <div class="w-8 h-8 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center shrink-0 text-xs font-black">{{ strtoupper(substr($navCat->name, 0, 2)) }}</div>
                            <div>
                                <p class="text-[13px] font-bold text-gray-900 leading-tight">{{ $navCat->name }}</p>
                                @if($navCat->activeSubcategories->isNotEmpty())
                                <p class="text-[10px] text-gray-500">{{ $navCat->activeSubcategories->pluck('name')->take(3)->join(', ') }}</p>
                                @endif
                            </div>
                        </a>
                        @endforeach
                        <a href="{{ url('/') }}#categories" class="block w-full text-center mt-4 text-xs font-bold text-primary hover:text-blue-700 transition-colors">View All Courses &rarr;</a>
                    </div>
                </div>

                <a href="{{ route('about') }}" class="block px-3 py-2.5 rounded-xl font-semibold text-gray-900 hover:bg-gray-50 hover:text-primary">About Us</a>
                <a href="{{ route('careers') }}" class="block px-3 py-2.5 rounded-xl font-semibold text-gray-900 hover:bg-gray-50 hover:text-primary">Careers</a>
                <a href="{{ route('faq') }}" class="block px-3 py-2.5 rounded-xl font-semibold text-gray-900 hover:bg-gray-50 hover:text-primary">FAQ</a>
                <a href="{{ route('contact') }}" class="block px-3 py-2.5 rounded-xl font-semibold text-gray-900 hover:bg-gray-50 hover:text-primary">Contact Us</a>
                ...
                <div class="pt-4 mt-3 border-t border-gray-100 space-y-2">
                    ...
                    @auth
                        <a href="{{ route('dashboard') }}" class="block px-3 py-2.5 rounded-xl font-bold text-white nav-cta-primary text-center">Dashboard</a>
                        <a href="{{ route('logout') }}" class="w-full text-center block px-3 py-2.5 rounded-xl font-bold text-red-600 bg-red-50 hover:bg-red-100">Logout</a>
                    @else
                        <a href="{{ route('login') }}" class="block px-3 py-2.5 rounded-xl font-bold text-white nav-cta-primary text-center">Login / Register</a>
                    @endauth
                </div>
            </div>
            </div>
