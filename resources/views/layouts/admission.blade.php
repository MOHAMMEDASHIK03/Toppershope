...
@push('before-content')
    @if(session('trial_created'))
        ...
        <div class="mb-4 p-5 bg-primary-50 border border-primary-200 rounded-xl">
            <p class="text-primary-900 font-semibold text-sm mb-3">Trial access issued — share these credentials with the student:</p>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div class="bg-white rounded-lg p-3 border border-primary-100">
                    <p class="text-primary-600 text-xs font-semibold mb-0.5">Login URL</p>
                    <p class="text-sm font-mono font-semibold text-slate-800">{{ url('/trial/login') }}</p>
                </div>
                <div class="bg-white rounded-lg p-3 border border-primary-100">
                    <p class="text-primary-600 text-xs font-semibold mb-0.5">Email</p>
                    <p class="text-sm font-mono font-semibold text-slate-800">{{ $tc['email'] ?? '' }}</p>
                </div>
                <div class="bg-white rounded-lg p-3 border border-primary-100">
                    <p class="text-primary-600 text-xs font-semibold mb-0.5">Password</p>
                    <p class="text-sm font-mono font-semibold text-slate-800">{{ $tc['password'] ?? '' }}</p>
                </div>
                <div class="bg-white rounded-lg p-3 border border-primary-100">
                    <p class="text-primary-600 text-xs font-semibold mb-0.5">Expires</p>
