...
<div class="py-4">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        @foreach([
            ['label' => 'Total Campaigns', 'value' => $totalCampaigns,  'icon' => 'megaphone',    'bg' => 'bg-primary-50',  'ic' => 'text-primary-500'],
            ['label' => 'Active Campaigns','value' => $activeCampaigns, 'icon' => 'check-circle',  'bg' => 'bg-green-50',   'ic' => 'text-green-500'],
            ['label' => 'Total Leads',     'value' => $totalLeads,      'icon' => 'users',         'bg' => 'bg-primary-50',    'ic' => 'text-primary-500'],
            ['label' => 'Enrol Interest',  'value' => $enrolLeads,      'icon' => 'graduation-cap','bg' => 'bg-primary-50',  'ic' => 'text-primary-500'],
        ] as $stat)
    ...
    <div class="mb-8 p-4 rounded-2xl border {{ $popup->is_active ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200' }} flex items-center justify-between gap-4">
        ...
        <a href="{{ route('ads.popups.index') }}" class="px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 hover:border-primary-500 hover:text-primary-700 transition">
            Manage Popup →
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                ...
                <a href="{{ route('ads.campaigns.index') }}" class="text-xs text-primary-500 font-bold hover:underline">View All →</a>
            </div>
            <div class="divide-y divide-slate-100">
                ...
                @empty
                <p class="px-6 py-8 text-sm text-slate-400 text-center">No campaigns yet. <a href="{{ route('ads.campaigns.create') }}" class="text-primary-500 font-bold">Create one →</a></p>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                ...
                <a href="{{ route('ads.leads.index') }}" class="text-xs text-primary-500 font-bold hover:underline">View All →</a>
            </div>
            <div class="divide-y divide-slate-100">
                ...
                <div class="px-6 py-3 flex items-center gap-4">
                    ...
                    <span class="px-2 py-1 rounded-md text-xs font-bold {{ $lead->enquiry_type === 'enrol' ? 'bg-primary-100 text-primary-700' : 'bg-primary-100 text-primary-700' }} shrink-0">
