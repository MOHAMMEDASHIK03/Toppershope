...
@section('title', ($category->landing('title', $category->name)) . ' Courses')
...
{{-- ===== HERO ===== --}}
<section class="relative bg-gradient-to-br {{ $heroBg }} border-b border-gray-200 py-20 overflow-hidden" id="cat-hero">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col md:flex-row items-center gap-12">
        <div class="md:w-2/3">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                <a href="{{ route('category.show', $category) }}" class="px-3 py-1 rounded-full border text-xs font-bold {{ !$subcategory ? 'bg-primary text-white border-primary' : $c['pill'] }}">All</a>
                @foreach($category->activeSubcategories as $sub)
                <a href="{{ route('category.show.subcategory', [$category, $sub]) }}" class="px-3 py-1 rounded-full border text-xs font-bold {{ $subcategory && $subcategory->id === $sub->id ? 'bg-primary text-white border-primary' : $c['pill'] }}">{{ $sub->name }}</a>
                @endforeach
            </div>
            <h1 class="text-5xl md:text-6xl font-black text-gray-900 mb-4">{{ $category->landing('title', $category->name) }}</h1>
            <p class="text-xl text-gray-600 mb-6 font-medium">{{ $category->landing('subtitle', '') }}</p>
            <p class="text-gray-600 leading-relaxed max-w-2xl mb-8">{{ $category->landing('about', $category->description) }}</p>
            @if($subcategory)
            <p class="text-sm font-bold text-primary mb-4">Showing batches for: {{ $subcategory->name }}</p>
            @endif
            <div class="flex flex-wrap gap-4">
                <a href="{{ route('register') }}" class="px-8 py-3.5 {{ $c['btn'] }} text-white font-bold rounded-xl transition-all shadow-lg hover:shadow-xl">
                    Enroll Now
                </a>
        ...
        <div class="md:w-1/3 flex justify-center md:justify-end">
            <div class="relative w-56 h-56 {{ $c['bg_light'] }} rounded-full flex items-center justify-center shadow-inner ring-8 {{ $c['ring'] }} ring-opacity-30">
                @if($iconUrl)
                <img src="{{ $iconUrl }}" alt="{{ $category->name }}" class="w-32 h-32 object-contain drop-shadow-xl">
                @endif
            ...
            <div class="gsap-reveal">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-4 bg-white p-4 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                        <div class="w-10 h-10 {{ $c['bg_light'] }} {{ $c['text'] }} rounded-lg flex items-center justify-center font-black text-sm shrink-0">{{ $loop->iteration }}</div>
...
<section id="batches" class="py-20 bg-white border-t border-gray-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 gsap-stagger-parent mb-16">
            ...
            <div class="gsap-stagger-child bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg transition-all group overflow-hidden flex flex-col">
                <div class="h-36 bg-gradient-to-br {{ $heroBg }} flex items-center justify-center border-b border-gray-100 relative">
                    ...
                    <span class="text-4xl font-black {{ $c['text'] }} uppercase tracking-tighter">{{ strtoupper(substr($batch->name,0,4)) }}</span>
                ...
                <div class="p-6 flex flex-col flex-grow">
                    <a href="{{ route('course.detail', $batch->course->slug) }}" class="font-bold text-gray-900 mb-1 group-hover:text-primary transition-colors text-lg hover:underline underline-offset-2 block">{{ $batch->name }}</a>
                    @if($batch->batch_description)
                    <p class="text-xs text-gray-500 mb-1">{{ $batch->batch_description }}</p>
                    @endif
                    ...
                    <div class="mb-3">
                        ...
                        <div class="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div class="h-full rounded-full {{ $batch->fill_percent >= 80 ? 'bg-red-400' : 'bg-primary' }}" style="width:{{ $batch->fill_percent }}%"></div>
                        </div>
                    </div>
                    @endif
                    <div class="mt-auto flex items-center justify-between border-t border-gray-100 pt-4">
                        <div>
                            @if($batch->original_price && $batch->original_price > 0)
                        ...
                        <a href="{{ route('checkout.show', $batch->uuid) }}" class="px-5 py-2.5 rounded-xl border-2 border-primary text-primary hover:bg-primary hover:text-white text-sm font-bold transition-all">Enroll Now</a>
                    </div>
                </div>
            </div>
            @endforeach
        ...
        {{-- Coming Soon Batches — fully DB-driven --}}
        ...
        <div class="gsap-reveal">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="relative bg-white rounded-2xl border-2 border-amber-200 p-6 shadow-sm hover:shadow-lg transition-all flex flex-col gap-3 overflow-hidden group">
                    ...
                    <div class="w-14 h-14 {{ $c['bg_light'] }} rounded-2xl flex items-center justify-center shrink-0">
                        @if($iconUrl)<img src="{{ $iconUrl }}" alt="{{ $ub->name }}" class="w-8 h-8 object-contain">@endif
                    </div>
                    <div>
                        <h4 class="font-bold text-gray-900 text-base mb-0.5">{{ $ub->name }}</h4>
                        @if($ub->batch_description)
                        <p class="text-xs text-gray-500">{{ $ub->batch_description }}</p>
                        @endif
                    </div>
                    @if($ub->start_date)
                    ...
                    <div class="flex items-center justify-between mt-1">
                        ...
                        <a href="{{ route('contact') }}" class="px-4 py-2 text-xs font-bold {{ $c['btn'] }} text-white rounded-xl transition-all">Register Interest</a>
                    </div>
                </div>
                @endforeach
            </div>
...
<section class="py-16 bg-gray-900">
    <div class="max-w-4xl mx-auto px-4 text-center gsap-reveal">
        <h2 class="text-3xl md:text-4xl font-black text-white mb-4">Ready to start your {{ $category->name }} journey?</h2>
        <p class="text-gray-400 mb-8 text-lg">Join thousands of students already studying smarter with Topper's Hope.</p>
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="{{ route('register') }}" class="px-10 py-4 {{ $c['btn'] }} text-white font-black rounded-xl text-lg transition-all shadow-xl">Start Free Demo</a>
            <a href="{{ route('faq') }}" class="px-10 py-4 bg-white/10 hover:bg-white/20 border border-white/20 text-white font-bold rounded-xl transition-all">Read FAQs</a>
        </div>
    </div>
</section>
