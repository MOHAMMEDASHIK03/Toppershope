        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            @foreach($enrollments as $enrollment)
                <a href="{{ route('student.my-courses.show', $enrollment->id) }}"
                   class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden hover:border-orange-200 hover:shadow-md transition-all group block">
                    <div class="h-40 bg-gradient-to-br from-orange-50 to-amber-50 relative overflow-hidden">
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center">
                                <span class="text-4xl font-bold text-orange-200">{{ strtoupper(substr($enrollment->batch->course->name, 0, 2)) }}</span>
                            </div>
                        @endif
                    ...
                    <div class="p-5">
                        <h3 class="font-semibold text-slate-900 text-sm group-hover:text-orange-600 transition-colors">{{ $enrollment->batch->course->name }}</h3>
                        <div class="flex items-center gap-2 text-xs text-slate-500 mt-2 flex-wrap">
                            @if($enrollment->batch->course->category)
                                <span class="px-2 py-0.5 rounded bg-orange-50 text-orange-700 font-semibold uppercase text-[10px]">{{ $enrollment->batch->course->category->name }}</span>
                            @endif
                        ...
                        <div class="mt-4 flex items-center justify-between">
                            ...
                            <span class="text-orange-600 text-xs font-semibold group-hover:text-orange-700">Continue →</span>
                        </div>
                    </div>
                </a>
            @endforeach
