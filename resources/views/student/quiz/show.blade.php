...
<div class="max-w-3xl mx-auto">
    <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-8 text-center">
        <div class="w-20 h-20 rounded-2xl bg-primary-500/10 flex items-center justify-center mx-auto mb-6">
            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="text-primary-700" viewBox="0 0 256 256"><path d="M200,24H56A16,16,0,0,0,40,40V216a16,16,0,0,0,16,16H200a16,16,0,0,0,16-16V40A16,16,0,0,0,200,24Zm-36,96H92a8,8,0,0,1,0-16h72a8,8,0,0,1,0,16Zm0-32H92a8,8,0,0,1,0-16h72a8,8,0,0,1,0,16Z"/></svg>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div class="bg-slate-50 rounded-xl p-3">
                <p class="text-lg font-black text-primary-700">{{ $quiz->sections->count() }}</p>
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 text-left mb-8">
            ...
            <ul class="space-y-2 text-xs text-slate-600">
                <li class="flex items-start gap-2"><span class="text-primary-700 mt-0.5">●</span> The quiz will open in <strong class="text-slate-900">fullscreen mode</strong>. Do NOT exit fullscreen or switch tabs.</li>
                ...
                <li class="flex items-start gap-2"><span class="text-primary-700 mt-0.5">●</span> Click <strong class="text-slate-900">Save & Next</strong> to save and move forward. Use <strong class="text-slate-900">Clear Response</strong> to deselect.</li>
        ...
        <div class="flex items-center justify-center gap-4">
            ...
            <a href="{{ route('student.quiz.attempt', $quiz->id) }}" target="_blank"
               class="px-8 py-3 rounded-xl bg-gradient-to-r from-primary-600 to-primary-600 text-white font-bold text-sm hover:from-primary-500 hover:to-primary-500 transition-all shadow-lg shadow-primary-500/20 inline-flex items-center gap-2">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 mt-6" id="attempts">
            ...
            <div class="space-y-3">
                @foreach($attempts as $idx => $attempt)
                    ...
                    @endphp
                    <div class="flex items-center gap-4 p-4 rounded-xl bg-slate-100/40 border border-slate-700/30 hover:border-primary-200 transition-all">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm
                            {{ $pct >= 70 ? 'bg-primary-500/15 text-emerald-400' : ($pct >= 40 ? 'bg-amber-500/15 text-amber-400' : 'bg-primary-500/15 text-rose-400') }}">
                            #{{ $attempts->count() - $idx }}
                        </div>
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <span class="text-xs font-bold px-2 py-0.5 rounded-md
                                    {{ $pct >= 70 ? 'bg-primary-50 text-emerald-700' : ($pct >= 40 ? 'bg-amber-500/10 text-amber-400' : 'bg-primary-500/10 text-rose-400') }}">
                                    {{ $pct }}%
                                </span>
                            </div>
                        ...
                        <div class="w-24 h-1.5 rounded-full bg-slate-700">
                            <div class="h-full rounded-full {{ $pct >= 70 ? 'bg-primary-500' : ($pct >= 40 ? 'bg-amber-500' : 'bg-primary-500') }}" style="width:{{ $pct }}%"></div>
                        </div>
                        <a href="{{ route('student.quiz.results', ['quiz' => $quiz->id, 'attempt' => $attempt->id]) }}"
                           class="px-3 py-2 rounded-lg bg-primary-50 text-primary-700 text-xs font-bold hover:bg-primary-500/20 transition-all">
                            View Details
                        </a>
                    </div>
                @endforeach
