...
<div class="mb-8">
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('student.my-courses') }}" class="text-orange-600 text-sm font-semibold hover:text-orange-700 transition-colors">View All →</a>
    </div>

    @if($enrollments->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            @foreach($enrollments->take(6) as $enrollment)
                <a href="{{ route('student.my-courses.show', $enrollment->id) }}" class="bg-white border border-slate-200 rounded-xl overflow-hidden hover:border-orange-200 hover:shadow-md transition-all group block shadow-sm">
                    <div class="h-36 bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center relative overflow-hidden">
                        ...
                        @else
                            <div class="text-4xl font-bold text-indigo-300">{{ strtoupper(substr($enrollment->batch->course->name, 0, 2)) }}</div>
                        @endif
                    ...
                    <div class="p-4">
                        <h3 class="font-semibold text-slate-900 text-sm mb-1 group-hover:text-orange-600 transition-colors">{{ $enrollment->batch->course->name }}</h3>
                        <div class="flex items-center gap-3 text-xs text-slate-500">
                            @if($enrollment->batch->course->category)
                                <span class="px-2 py-0.5 rounded bg-orange-50 text-indigo-700 font-semibold">{{ strtoupper($enrollment->batch->course->category->name) }}</span>
                            @endif
            ...
            <tbody>
                @foreach($recentQuizzes as $attempt)
                    <tr>
                        ...
                        <td class="text-right">
                            <a href="{{ route('student.quiz.results', $attempt->quiz_id) }}" class="text-orange-600 text-xs font-semibold hover:text-orange-700">View Results →</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
