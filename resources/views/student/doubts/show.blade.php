...
@section('content')
<div class="max-w-3xl">
    <a href="{{ route('student.doubts') }}" class="text-sm text-slate-500 hover:text-orange-600 transition-colors mb-6 inline-block">← Back to Doubts</a>
    ...
    <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 mb-6">
        ...
        @if($doubt->faculty_reply)
            <div class="mt-4 p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
    ...
    @if($doubt->replies && $doubt->replies->count() > 0)
        ...
        <div class="space-y-3 mb-6">
            @foreach($doubt->replies as $reply)
                <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-4 {{ $reply->user_id === auth()->id() ? 'border-l-4 border-l-indigo-500' : 'border-l-4 border-l-emerald-500' }}">
                    <div class="flex items-center gap-2 mb-2">
                        <span class="text-xs font-bold {{ $reply->user_id === auth()->id() ? 'text-orange-600' : 'text-emerald-400' }}">
    ...
    <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
        <form action="{{ route('student.doubts.reply', $doubt->id) }}" method="POST">
            ...
            <textarea name="reply_text" rows="3" required placeholder="Type your reply..."
                class="w-full px-4 py-3 admin-input rounded-xl text-sm resize-none outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 resize-none mb-3"></textarea>
