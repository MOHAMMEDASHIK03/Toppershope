...
@if(isset($globalPopup) && $globalPopup->is_active && $globalPopup->image)
<div id="global-popup-overlay" style="display:none"
    ...
    <div class="relative max-w-sm w-full bg-white rounded-3xl overflow-hidden shadow-2xl animate-pulse-once">
        ...
        @if($globalPopup->link_url)
        <a href="{{ $globalPopup->link_url }}" target="_blank" onclick="document.getElementById('global-popup-overlay').style.display='none'">
            <img src="{{ Storage::url($globalPopup->image) }}" alt="Special Offer" class="w-full object-cover">
        </a>
        @if($globalPopup->link_text)
        <div class="p-4 text-center">
            <a href="{{ $globalPopup->link_url }}" target="_blank"
               class="inline-block px-6 py-2.5 bg-primary text-white font-black rounded-xl text-sm transition hover:bg-primary-700">
                {{ $globalPopup->link_text }}
            </a>
        </div>
        @endif
        @else
...
<script>
    ...
    }, {{ ($globalPopup->delay_seconds ?? 3) * 1000 }});
</script>
@endif

{{-- ===== 0. SCROLLING AD CAROUSEL ===== --}}
<div x-data="{
    activeSlide: 0,
    progress: 0,
    timer: null,
    slides: [
        { color: 'from-[#7723D6] via-[#7723D6] to-[#581C87]', badge: 'Limited Time Offer', title: 'UP TO 55% OFF', sub: 'On JEE & NEET flagship batches. Scholarship test active now.', cta: 'Grab Offer', href: '#scholarship-test' },
        { color: 'from-[#805AD5] via-[#6f52f0] to-[#5035d5]', badge: 'Early Access 2026', title: 'NEW ACADEMIC BATCHES', sub: 'Live classes starting soon. Book a free demo and reserve your seat.', cta: 'Book Demo', href: '#demo-session' },
        { color: 'from-primary-600 via-primary-700 to-primary-800', badge: 'National Scholarship', title: 'SCHOLARSHIP TEST LIVE', sub: 'Get up to 90% fee waiver. Register free and prove your potential.', cta: 'Register', href: '{{ route('register') }}' }
    ],
    startProgress() {
...
}" x-init="init()" @mouseenter="clearInterval(timer)" @mouseleave="startProgress()" class="relative w-full group">
    <template x-for="(slide, i) in slides" :key="i">
        <div x-show="activeSlide === i"
            ...
            class="w-full min-h-[5.5rem] sm:min-h-[6.5rem] md:min-h-[7.5rem] flex items-center justify-between px-5 sm:px-10 lg:px-24 py-3 sm:py-4 bg-gradient-to-r absolute inset-0"
            :class="slide.color">
            <div class="flex-1 text-white min-w-0 pr-3">
                <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-5">
                    <div class="min-w-0">
                        ...
                        <h2 class="text-base sm:text-xl md:text-2xl lg:text-3xl font-black uppercase tracking-tight leading-tight" x-text="slide.title"></h2>
                    </div>
                    <p class="text-xs sm:text-sm text-white/85 max-w-sm hidden md:block" x-text="slide.sub"></p>
                </div>
            </div>
            <a :href="slide.href" class="shrink-0 px-4 py-2 md:px-5 md:py-2.5 bg-white text-slate-900 font-extrabold rounded-full text-xs md:text-sm transition-all shadow-lg hover:-translate-y-0.5 whitespace-nowrap" x-text="slide.cta"></a>
        </div>
    </template>
    {{-- Height placeholder --}}
    <div class="min-h-[5.5rem] sm:min-h-[6.5rem] md:min-h-[7.5rem] pointer-events-none" aria-hidden="true"></div>
    {{-- Dots --}}
...
{{-- ===== 1. HERO SECTION with GSAP + Floating Cards ===== --}}
<section id="hero" class="relative py-14 sm:py-20 lg:py-24 bg-[linear-gradient(180deg,#F5F0FF_0%,#ffffff_45%,#F5F0FF_100%)] overflow-hidden">
    <div class="absolute top-0 right-0 w-[30rem] h-[30rem] rounded-full bg-primary-100/70 blur-3xl opacity-60 -translate-y-1/2 translate-x-1/3 z-0"></div>
    <div class="absolute bottom-0 left-0 w-[24rem] h-[24rem] rounded-full bg-primary-100/70 blur-3xl opacity-70 translate-y-1/3 -translate-x-1/4 z-0"></div>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            {{-- LEFT --}}
            <div>
                <div id="hero-badge" class="inline-flex items-center gap-2 py-1.5 px-3.5 rounded-full bg-primary-50 border border-primary-100 text-xs sm:text-sm font-bold text-primary mb-6 shadow-sm">
                ...
                <h1 id="hero-title" class="text-4xl sm:text-5xl lg:text-6xl tracking-tight font-black text-gray-900 mb-5 leading-[1.04]">
                    ...
                    <span class="bg-gradient-to-r from-[#7723D6] via-primary-700 to-primary-800 bg-clip-text text-transparent">
                ...
                <div id="hero-btns" class="flex flex-col sm:flex-row gap-4">
                    <a href="#categories" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3.5 rounded-xl text-white font-extrabold text-base bg-gradient-to-r from-[#7723D6] to-[#6B21C8] shadow-[0_8px_30px_rgba(119,35,214,0.30)] hover:shadow-[0_12px_34px_rgba(119,35,214,0.38)] hover:-translate-y-0.5 transition-all">
                        Explore Courses
                    </a>
                ...
                <div id="hero-social" class="mt-7 flex flex-wrap items-center gap-4 sm:gap-6">
                    <div class="flex -space-x-3">
                        <div class="w-10 h-10 rounded-full bg-primary-100 border-2 border-white flex items-center justify-center text-xs font-bold text-primary-700">RS</div>
                        <div class="w-10 h-10 rounded-full bg-primary-100 border-2 border-white flex items-center justify-center text-xs font-bold text-primary-700">MP</div>
                        ...
                        <div class="w-10 h-10 rounded-full bg-primary-100 border-2 border-white flex items-center justify-center text-[10px] font-bold text-primary-700">1M+</div>
                    </div>
                ...
                <div class="relative p-4 sm:p-5 rounded-[2rem] bg-white/70 border border-white/80 shadow-[0_20px_60px_rgba(30,41,59,0.12)] backdrop-blur-md">
                    <div class="rounded-[1.5rem] overflow-hidden border border-gray-200 bg-white shadow-sm">
                        <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                            ...
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-primary-50 text-emerald-700 border border-emerald-200">
                        ...
                        <div class="p-4 grid grid-cols-3 gap-3">
                            <div class="rounded-xl bg-primary-50 border border-primary-100 p-3">
                                <p class="text-xl font-black text-primary leading-none">98%</p>
                                <p class="text-[10px] font-bold uppercase tracking-wide text-primary-700 mt-1">Success Rate</p>
                            </div>
                            <div class="rounded-xl bg-primary-50 border border-primary-100 p-3">
                                <p class="text-xl font-black text-primary-600 leading-none">AIR 47</p>
                                <p class="text-[10px] font-bold uppercase tracking-wide text-primary-700 mt-1">Top Rank</p>
                            </div>
                            <div class="rounded-xl bg-primary-50 border border-emerald-100 p-3">
                                <p class="text-xl font-black text-primary-600 leading-none">12.8k</p>
...
<div class="relative py-6 bg-gradient-to-b from-slate-50 to-white border-y border-slate-200/80 overflow-x-auto">
    <div class="absolute inset-x-0 top-0 h-20 bg-[radial-gradient(circle_at_center,rgba(119,35,214,0.10),transparent_65%)] pointer-events-none"></div>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div class="flex gap-2.5 sm:gap-3 items-center justify-start md:justify-center min-w-max mx-auto">
            ...
            <a href="{{ route('category.show', $pillCategory) }}" class="px-4 sm:px-5 py-2 rounded-full {{ $loop->first ? 'bg-gradient-to-r from-primary to-primary-600 text-white border border-primary-600 shadow-[0_6px_20px_rgba(119,35,214,0.35)]' : 'bg-white/90 backdrop-blur border border-slate-200 text-slate-600 hover:text-primary hover:border-primary-200 hover:shadow-md' }} font-bold text-xs sm:text-sm transition-all whitespace-nowrap">
                {{ $pillCategory->name }}
            </a>
            @endforeach
        </div>
...
{{-- ===== 2. EXAM CATEGORIES PREMIUM ===== --}}
<section id="categories" class="relative py-20 bg-[linear-gradient(180deg,#F5F0FF_0%,#ffffff_55%,#F5F0FF_100%)] overflow-hidden">
    <div class="absolute top-0 right-0 w-[28rem] h-[28rem] bg-primary-100/60 blur-3xl rounded-full -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
    <div class="absolute bottom-0 left-0 w-[24rem] h-[24rem] bg-primary-100/60 blur-3xl rounded-full translate-y-1/2 -translate-x-1/3 pointer-events-none"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5 gsap-stagger-parent">
            ...
            <a href="{{ route('category.show', $cat) }}" class="gsap-stagger-child group relative rounded-2xl p-[1px] bg-gradient-to-br from-white via-primary-100/70 to-primary-100/80 hover:from-primary-200 hover:to-primary-200 transition-all duration-300">
                <div class="relative h-full min-h-[205px] rounded-2xl border border-white/80 bg-white/85 backdrop-blur-sm shadow-[0_12px_30px_rgba(15,23,42,0.06)] group-hover:shadow-[0_20px_35px_rgba(119,35,214,0.16)] group-hover:-translate-y-1 transition-all duration-300 p-5 flex flex-col">
                    <div class="absolute -top-8 -right-8 w-24 h-24 bg-gradient-to-br from-primary-100/60 to-primary-100/60 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>

                    <div class="mb-4">
                        <div class="w-12 h-12 rounded-xl bg-gradient-to-br {{ $cat->landing('icon_bg', 'from-primary-500 to-primary-500') }} text-white shadow-md flex items-center justify-center shrink-0">
                            @if($cat->landing('icon_svg'))
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="{{ $cat->landing('icon_svg') }}"></path>
                            </svg>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        <h3 class="text-lg font-black text-slate-900 group-hover:text-primary transition-colors leading-tight">{{ $cat->name }}</h3>
                    </div>

                    <div class="flex flex-wrap gap-1.5 mb-5">
                        @foreach($cat->activeSubcategories->take(3) as $sub)
                        <span class="px-2.5 py-1 rounded-full text-[11px] font-semibold text-slate-600 bg-slate-50 border border-slate-200">{{ $sub->name }}</span>
                        @endforeach
                    </div>
...
{{-- ===== 3. LIVE BATCHES from DB ===== --}}
<section class="relative py-20 bg-[linear-gradient(180deg,#ffffff_0%,#F5F0FF_100%)] border-t border-gray-100 overflow-hidden">
    <div class="absolute top-0 right-0 w-[26rem] h-[26rem] bg-primary-100/60 blur-3xl rounded-full -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
    <div class="absolute bottom-0 left-0 w-[22rem] h-[22rem] bg-primary-100/60 blur-3xl rounded-full translate-y-1/2 -translate-x-1/3 pointer-events-none"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div class="flex items-end justify-between mb-10 gsap-reveal">
            ...
            <a href="#" class="hidden sm:inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-extrabold text-primary border border-primary-200 bg-white hover:bg-primary-50 transition-colors shadow-sm">
                View All
...
{{-- ===== 4. UPCOMING BATCHES from DB ===== --}}
@if(isset($upcomingBatches) && $upcomingBatches->count() > 0)
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 gsap-stagger-parent">
            ...
            <div class="gsap-stagger-child relative bg-white rounded-2xl border-2 border-amber-200 p-6 shadow-sm hover:shadow-lg transition-all flex flex-col gap-3 overflow-hidden group">
                ...
                <div>
                    <a href="{{ route('course.detail', $ub->course->slug) }}" class="font-bold text-gray-900 text-base hover:text-primary transition-colors block mb-0.5">{{ $ub->name }}</a>
                    @if($ub->batch_description)
                    <p class="text-xs text-gray-500 mt-0.5">{{ $ub->batch_description }}</p>
                    @elseif($ub->course->description)
                    <p class="text-xs text-gray-500 mt-0.5 line-clamp-2">{{ $ub->course->description }}</p>
                    @endif
                </div>
                @if($ub->start_date)
                <div class="flex items-center gap-1.5 text-xs text-gray-500">
                    ...
                    Starting {{ $ub->start_date->format('d M Y') }}
                </div>
                @endif
                @if($ub->mode)
                ...
                <div class="flex items-center justify-between mt-auto border-t border-gray-100 pt-3">
                    ...
                    <a href="{{ route('contact') }}" class="px-4 py-2 text-xs font-bold bg-amber-500 hover:bg-amber-600 text-white rounded-xl transition-colors">
                        Register Interest
                    </a>
                </div>
            </div>
...
<section id="scholarship-test" class="py-12 bg-slate-50 border-t border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="gsap-reveal relative rounded-3xl overflow-hidden border border-primary-200/70 bg-gradient-to-r from-primary-50 via-primary-50 to-primary-50 shadow-[0_14px_35px_rgba(119,35,214,0.14)]">
            <div class="absolute -top-20 -right-12 w-64 h-64 bg-primary-200/40 rounded-full blur-3xl"></div>
            <div class="grid md:grid-cols-2 items-center gap-8 px-6 py-8 md:px-10 md:py-10 relative">
                <div>
                    <span class="inline-flex items-center gap-2 bg-primary-100 text-primary-700 text-xs font-black px-3 py-1 rounded-full uppercase tracking-widest mb-4 border border-primary-200">
                    ...
                    <a href="{{ route('register') }}" class="inline-flex items-center gap-2 px-7 py-3 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl transition-all shadow-[0_8px_22px_rgba(15,23,42,0.30)]">
                        Enroll Now
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                    </a>
                </div>
...
{{-- ===== 6. DEMO SESSION BANNER ===== --}}
<section id="demo-session" class="py-12 bg-white border-t border-gray-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="gsap-reveal relative rounded-3xl overflow-hidden border border-primary-200/70 bg-gradient-to-r from-primary-50 via-primary-50 to-violet-50 shadow-[0_14px_35px_rgba(119,35,214,0.16)]">
            <div class="absolute -bottom-20 -left-10 w-64 h-64 bg-primary-200/40 rounded-full blur-3xl"></div>
            <div class="grid md:grid-cols-2 items-center gap-8 px-6 py-8 md:px-10 md:py-10 relative">
                ...
                <div class="order-1 md:order-2">
                    ...
                    <h3 class="text-3xl md:text-4xl font-black text-slate-900 leading-tight mb-3">Book Your <span class="text-primary-600">10-Day Demo Session</span></h3>
                    ...
                    <a href="{{ route('contact') }}" class="inline-flex items-center gap-2 px-7 py-3 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl transition-all shadow-[0_8px_22px_rgba(119,35,214,0.35)]">
                        Book Now
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                    </a>
                </div>
...
<section class="py-12 bg-slate-50 border-t border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="gsap-reveal relative rounded-3xl overflow-hidden border border-primary-200/70 bg-gradient-to-r from-primary-50 via-primary-50 to-primary-50 shadow-[0_14px_35px_rgba(168,85,247,0.16)]">
            <div class="absolute -top-24 -right-14 w-64 h-64 bg-primary-200/40 rounded-full blur-3xl"></div>
            <div class="grid md:grid-cols-2 items-center gap-8 px-6 py-8 md:px-10 md:py-10 relative">
                <div>
                    <span class="inline-flex items-center gap-2 bg-primary-100 text-primary-700 text-xs font-black px-3 py-1 rounded-full uppercase tracking-widest mb-4 border border-primary-200">
                    ...
                    <h3 class="text-3xl md:text-4xl font-black text-slate-900 leading-tight mb-3">Talk to Our <span class="text-primary-600">Academic Experts</span></h3>
                    ...
                    <a href="{{ route('contact') }}" class="inline-flex items-center gap-2 px-7 py-3 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl transition-all shadow-[0_8px_22px_rgba(192,38,211,0.35)]">
                        Request Callback
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                    </a>
                </div>
...
{{-- ===== 7. WHY CHOOSE US ===== --}}
<section class="relative py-20 bg-[linear-gradient(180deg,#ffffff_0%,#F5F0FF_100%)] border-t border-gray-100 overflow-hidden">
    <div class="absolute top-0 right-0 w-[24rem] h-[24rem] bg-primary-100/60 blur-3xl rounded-full -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 gsap-stagger-parent">
            @php
            $features = [
                ['icon'=>'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z','title'=>'Live Interactive Classes','desc'=>'Learn directly from expert educators with classroom-like engagement and doubt solving.','color'=>'from-primary-500 to-primary-500'],
                ['icon'=>'M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z M21 12a9 9 0 11-18 0 9 9 0 0118 0z','title'=>'Video Solutions','desc'=>'Detailed step-by-step explanations for assignments, tests, and practice sheets.','color'=>'from-primary-500 to-primary-500'],
                ['icon'=>'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z','title'=>'Affordable Premium','desc'=>'Top-tier educational quality at a student-friendly fee structure.','color'=>'from-primary-500 to-primary-500'],
                ['icon'=>'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z','title'=>'Trusted Curriculum','desc'=>'Exam-focused content aligned with latest standards and official patterns.','color'=>'from-primary-500 to-primary-500'],
                ...
                ['icon'=>'M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z','title'=>'24×7 Doubt Desk','desc'=>'Get academic support quickly whenever you are stuck during preparation.','color'=>'from-primary-500 to-primary-500'],
            ];
            @endphp

            @foreach($features as $f)
            <div class="gsap-stagger-child group rounded-2xl p-[1px] bg-gradient-to-br from-slate-100 via-primary-100/70 to-primary-100/80 hover:from-primary-200 hover:to-primary-200 transition-all">
                <div class="h-full rounded-2xl bg-white/90 border border-white/80 backdrop-blur-sm p-6 shadow-[0_10px_25px_rgba(15,23,42,0.06)] group-hover:shadow-[0_16px_30px_rgba(119,35,214,0.15)] group-hover:-translate-y-1 transition-all">
                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br {{ $f['color'] }} text-white flex items-center justify-center mb-4 shadow-md">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="{{ $f['icon'] }}"></path></svg>
                    </div>
                    <h3 class="text-base font-black text-slate-900 mb-2">{{ $f['title'] }}</h3>
...
}" class="relative py-20 bg-[linear-gradient(180deg,#f9f7ff_0%,#F5F0FF_100%)] border-t border-gray-100 overflow-hidden">
    <div class="absolute bottom-0 left-0 w-[22rem] h-[22rem] bg-primary-100/50 blur-3xl rounded-full translate-y-1/2 -translate-x-1/3 pointer-events-none"></div>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6 gsap-reveal">
            <div>
                <span class="inline-flex items-center gap-2 text-[11px] font-black tracking-[0.14em] uppercase text-primary-700 bg-primary-100 border border-primary-200 rounded-full px-3 py-1 mb-4">
            ...
            <div class="inline-flex items-center gap-2 text-sm font-bold text-slate-700 bg-white border border-slate-200 rounded-full px-4 py-2 shadow-sm">
                ...
                {{ $testimonialAverageRating > 0 ? number_format($testimonialAverageRating, 1) : '0.0' }}/5 from {{ number_format($testimonialCount ?? 0) }} reviews
            </div>
        </div>
        ...
        <div class="relative">
            ...
            <div class="pointer-events-none absolute right-0 top-0 bottom-0 w-12 bg-gradient-to-l from-[#F5F0FF] to-transparent z-10"></div>
            ...
            <div x-ref="testimonialTrack" class="flex gap-4 overflow-x-auto scroll-smooth py-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                @foreach([1,2] as $copy)
                    ...
                    @php
                        ...
                        $avatarPalette = ['bg-primary-100', 'bg-primary-100', 'bg-green-100', 'bg-primary-100', 'bg-amber-100', 'bg-primary-100'];
                        $avatarClass = $avatarPalette[$loop->index % count($avatarPalette)];
                    @endphp
                    <div class="shrink-0 w-[88%] sm:w-[72%] md:w-[48%] lg:w-[31%] gsap-stagger-child group rounded-2xl p-[1px] bg-gradient-to-br from-slate-100 via-primary-100/60 to-primary-100/70 hover:from-primary-200 hover:to-violet-200 transition-all" @mouseenter="paused = true" @mouseleave="paused = false" @mousedown="paused = true" @mouseup="paused = false">
                        <div class="h-full rounded-2xl bg-white/92 border border-white/80 backdrop-blur p-5 shadow-[0_10px_25px_rgba(15,23,42,0.06)] group-hover:shadow-[0_16px_32px_rgba(119,35,214,0.16)] group-hover:-translate-y-1 transition-all flex flex-col gap-4">
                            <div class="flex items-start justify-between">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 {{ $avatarClass }} rounded-full flex items-center justify-center font-black text-xs text-slate-700">{{ $initial }}</div>
                                    <div>
                                        <h4 class="font-black text-slate-900 text-sm">{{ $employeeName }}</h4>
                                        <p class="text-[11px] text-primary-600 font-bold">{{ $courseName }}</p>
                                    </div>
                                </div>
                                <span class="px-2.5 py-1 text-[10px] font-black rounded-full bg-primary-50 text-primary-700 border border-primary-100">{{ $rankOrScore }}</span>
                            </div>
                            ...
                            <div>
                                ...
                                @if($isLongText)
                                    <button type="button" class="mt-1.5 text-[11px] font-bold text-primary hover:text-primary-700" @click="paused = true; openReview(@js($employeeName), @js($courseName), {{ (int)($review->rating ?? 0) }}, @js($feedbackText))">
                                        Read more
                                    </button>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center gap-1">
                                @for($i=0;$i<5;$i++)
                                    <svg class="w-4 h-4 {{ $i < (int)($review->rating ?? 0) ? 'text-amber-500' : 'text-slate-300' }}" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.922-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.176 0l-2.8 2.034c-.784.57-1.838-.196-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                @endfor
                            </div>
                        </div>
                    </div>
        ...
        <div x-transition class="w-full max-w-xl rounded-2xl bg-white border border-slate-200 shadow-2xl p-6">
            <div class="flex items-start justify-between mb-4">
                <div>
                    ...
                    <p class="text-xs font-bold text-primary-600 mt-1" x-text="selectedReview.period"></p>
                </div>
