...
>
    <x-slot:fields>
        <div>
            ...
            <input type="text" name="otp" id="otp" value="{{ old('otp') }}" required autofocus
                ...
                placeholder="000000"
                class="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-lg tracking-[0.4em] text-center font-bold focus:ring-primary-600/15 focus:border-primary-500 outline-none text-slate-900">
    ...
    <x-slot:afterForm>
        <form action="{{ route('admin.password.forgot.resend') }}" method="POST" class="mt-4 text-center">
            @csrf
            <button type="submit" class="text-sm font-semibold text-primary-700 hover:text-primary-700">
                Resend code
            </button>
        </form>
        <p class="text-center mt-4 text-sm text-slate-500">
            <a href="{{ route('admin.password.forgot') }}" class="font-semibold text-primary-700 hover:text-primary-700">Use a different email</a>
        </p>
    </x-slot:afterForm>
</x-panel.auth-page>
