...
<x-admin.card>
    ...
    <form action="{{ route('admin.expenses.store') }}" method="POST" class="mb-6 p-4 bg-slate-50 dark:bg-[#141418] border border-slate-200 dark:border-[#2a2a32] rounded-xl">
        @csrf
                ...
                <tr>
                    <td>
                        <div class="flex items-center gap-2.5">
                            <span class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 {{ $tx->type === 'income' ? 'bg-primary-50 text-primary-600' : 'bg-rose-50 text-primary-600' }}">
                    ...
                    <td class="text-right font-semibold whitespace-nowrap {{ $tx->type === 'income' ? 'text-primary-600' : 'text-primary-600' }}">
