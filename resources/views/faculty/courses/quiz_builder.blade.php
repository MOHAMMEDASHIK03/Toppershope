...
@section('page_header')
    <div class="flex items-center gap-3">
        ...
        <span>Builder: <span class="text-orange-600">{{ $quiz->title }}</span></span>
    </div>
@endsection
...
<div x-data="{ addingSection: false, importingQuiz: false, activeQuestionSection: null }" class="max-w-5xl mx-auto pb-12">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        <div class="absolute top-0 right-0 w-64 h-64 bg-orange-50 rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none"></div>
        
        <div class="relative z-10 hidden md:block w-16 h-16 bg-orange-100 text-orange-600 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-inner">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row gap-3 mt-4 md:mt-0">
            ...
            <form action="{{ route('faculty.courses.quizzes.publish', [$course->id, $quiz->id]) }}" method="POST" class="inline">
                @csrf
                <button type="submit" class="{{ $quiz->is_published ? 'bg-amber-500 hover:bg-amber-600 shadow-amber-500/20' : 'btn-primary shadow-orange-500/20' }} text-white font-bold py-2 px-5 rounded-xl shadow-sm transition-all text-sm flex items-center justify-center gap-2 w-full">
    ...
    <div class="space-y-8">
        @forelse($quiz->sections as $index => $section)
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden ring-1 ring-slate-100">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-4 text-xs font-semibold text-slate-500 mt-1">
                                <span class="flex items-center gap-1" title="Time Limit"><i class="ph-bold ph-timer text-indigo-400"></i> {{ $section->time_limit_minutes ? $section->time_limit_minutes . ' Mins' : 'No Limit' }}</span>
                                <span class="flex items-center gap-1" title="Correct Marks"><i class="ph-bold ph-check-circle text-emerald-500"></i> +{{ $section->marks_per_question }}</span>
                ...
                <div class="p-6">
                    ...
                    @if($section->questions->count() > 0)
                        <div class="space-y-5 mb-6">
                            @foreach($section->questions as $qIndex => $question)
                                <div class="bg-white border text-sm border-slate-200 rounded-xl p-5 relative shadow-sm hover:border-indigo-200 transition-colors group">
                                    <!-- Delete Question Btn -->
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 pl-12 pr-4">
                                        @foreach($question->options as $optIndex => $option)
                                            <div class="relative flex items-start gap-3 p-3 rounded-lg border {{ $option->is_correct ? 'border-emerald-300 bg-emerald-50/50' : 'border-slate-100 bg-slate-50' }}">
                                                @if($option->is_correct)
                                                    <i class="ph-fill ph-check-circle text-emerald-500 text-lg relative top-0.5 flex-shrink-0"></i>
                                                @else
                    ...
                    <button x-show="activeQuestionSection != {{ $section->id }}" @click="activeQuestionSection = {{ $section->id }}" class="w-full py-4 border-2 border-dashed border-indigo-200 bg-orange-50/50 hover:bg-orange-50 rounded-xl text-orange-600 font-bold text-sm transition-colors flex items-center justify-center gap-2">
                        <i class="ph-bold ph-plus-circle text-lg"></i> Add Question to "{{ $section->name }}"
                    </button>

                    <!-- Add New Question Inline Form -->
                    <div x-show="activeQuestionSection == {{ $section->id }}" x-transition x-cloak class="border border-indigo-200 bg-orange-50/30 rounded-xl p-6 shadow-sm shadow-indigo-100/50" style="display: none;">
                        <div class="flex items-center justify-between mb-5 pb-3 border-b border-indigo-100">
                            <h4 class="font-bold text-slate-800 flex items-center gap-2"><i class="ph-fill ph-question text-indigo-500 text-lg"></i> Formulate Question</h4>
                        ...
                        <form action="{{ route('faculty.courses.quizzes.questions.store', [$course->id, $section->id]) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                                <div class="col-span-2 md:col-span-1">
                                    ...
                                    <textarea name="question_text" rows="3" placeholder="Type the question here..." class="w-full text-sm bg-white border border-slate-300 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-indigo-500 transition-colors resize-none shadow-sm"></textarea>
                                </div>
                                <div class="col-span-2 md:col-span-1" x-data="{ fileName: '' }">
                                    ...
                                    <div class="relative w-full h-24 rounded-lg overflow-hidden transition-colors" :class="fileName ? 'border border-indigo-200 bg-indigo-50/30' : 'border-2 border-dashed border-slate-300 bg-white hover:border-indigo-400'">
                                        ...
                                        <div x-show="fileName" style="display: none;" class="absolute inset-0 flex flex-col items-center justify-center p-4">
                                            <div class="flex items-center gap-2 max-w-full px-4">
                                                <i class="ph-fill ph-image text-indigo-500 text-xl flex-shrink-0"></i>
                                                <span class="text-sm font-semibold text-indigo-700 truncate" x-text="fileName"></span>
                                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                @for($i = 0; $i < 4; $i++)
                                    <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm focus-within:border-indigo-400 focus-within:ring-1 focus-within:ring-indigo-400 transition-all relative group">
                                        
                                        <!-- Correct Answer Radio -->
                                        <div class="absolute top-4 right-4 bg-slate-50 border border-slate-200 rounded px-2 py-1 shadow-sm flex items-center gap-2 z-10 cursor-pointer hover:bg-emerald-50 transition-colors group-focus-within:border-emerald-200">
                                            <input type="radio" name="correct_option" value="{{ $i }}" {{ $i == 0 ? 'required' : '' }} class="text-emerald-500 focus:ring-emerald-500 w-4 h-4 cursor-pointer">
                                            <label class="text-[10px] font-bold text-slate-600 uppercase tracking-widest cursor-pointer">True</label>
                                        </div>

                                        <div class="font-bold text-slate-800 mb-3 flex items-center gap-1.5"><i class="ph-fill ph-text-aa text-indigo-400"></i> Option {{ chr(65 + $i) }}</div>
                                        
                                        <div class="space-y-3">
                                            <textarea name="options[{{ $i }}][text]" rows="2" placeholder="Text for option {{ chr(65 + $i) }}..." class="w-full text-xs bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 outline-none focus:bg-white focus:border-indigo-300 transition-colors resize-none"></textarea>
                                            
                                            <div x-data="{ fileName: '' }" class="relative flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 focus-within:bg-white focus-within:border-indigo-300 transition-colors overflow-hidden">
                                                <i class="ph-fill ph-image flex-shrink-0" :class="fileName ? 'text-indigo-500' : 'text-slate-400'"></i>
                                                <div class="flex-1 min-w-0 relative">
                                                    ...
                                                    <div x-show="fileName" style="display: none;" class="text-xs font-semibold text-indigo-700 truncate pr-8 pt-1 pb-1" x-text="fileName"></div>
                                                </div>
                            ...
                            <div class="flex justify-end gap-3 pt-4 border-t border-indigo-100">
                                ...
                                <button type="submit" class="btn-primary text-white font-bold py-2 px-6 rounded-lg shadow-sm shadow-orange-500/20 transition-all text-sm flex items-center gap-2"><i class="ph-bold ph-plus"></i> Save Question</button>
                            </div>
                        </form>
                    </div>
        ...
        @empty
            <div class="text-center py-16 bg-white border border-dashed border-slate-300 rounded-2xl">
                <div class="w-20 h-20 bg-orange-50 text-indigo-200 rounded-full flex items-center justify-center text-4xl mx-auto mb-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
        ...
        <button x-show="!addingSection && !importingQuiz" @click="importingQuiz = true" class="w-full py-5 border-2 border-dashed border-indigo-300 hover:border-indigo-400 bg-orange-50/30 hover:bg-orange-50/60 rounded-2xl text-orange-600 font-bold text-sm transition-all flex items-center justify-center gap-2 shadow-sm">
            <i class="ph-bold ph-upload-simple text-xl text-indigo-500"></i> Bulk Import via ZIP
        </button>
    </div>

    <!-- Create Section Form Modal -->
    <div x-show="addingSection" x-transition x-cloak class="mt-6 bg-white p-6 md:p-8 rounded-2xl border border-indigo-200 shadow-xl relative" style="display: none;">
        ...
        <h3 class="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2"><i class="ph-fill ph-squares-four text-indigo-500"></i> New Section Configuration</h3>
        ...
        <form action="{{ route('faculty.courses.quizzes.sections.store', [$course->id, $quiz->id]) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <input type="text" name="name" required placeholder="e.g. Physics Core Concepts" class="w-full text-sm bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-indigo-500 transition-colors">
                </div>
                ...
                <div class="col-span-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="time_limit_minutes" min="1" placeholder="Leave blank for no limit" class="w-full pl-11 text-sm bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-indigo-500 transition-colors">
                    </div>
                </div>
            </div>

            <div class="bg-orange-50/50 border border-indigo-100 rounded-xl p-5 mb-6">
                <h4 class="text-xs font-bold text-indigo-800 uppercase tracking-widest mb-4">Scoring Schema</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <!-- Marks per Q -->
                    <div class="col-span-1">
                        <label class="block text-xs font-bold text-slate-700 mb-2">Positive Marks</label>
                        <div class="relative">
                            <i class="ph-bold ph-check-circle absolute left-4 top-1/2 -translate-y-1/2 text-emerald-500 text-lg"></i>
                            <input type="number" step="0.5" name="marks_per_question" value="4" required class="w-full pl-11 font-bold text-sm bg-white border border-slate-200 rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-indigo-500 transition-colors">
                        </div>
                    </div>
                    <!-- Negative Marks -->
                    <div class="col-span-1">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.5" name="negative_marks_per_question" value="1" required class="w-full pl-11 font-bold text-sm bg-white border border-slate-200 rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-indigo-500 transition-colors">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="btn-primary text-white font-bold py-2.5 px-8 rounded-xl shadow-md shadow-orange-500/20 transition-all text-sm flex items-center gap-2">Create Section</button>
            </div>
        </form>
    </div>

    <!-- Bulk Import Form Modal -->
    <div x-show="importingQuiz" x-transition x-cloak class="mt-6 bg-white p-6 md:p-8 rounded-2xl border border-indigo-200 shadow-xl relative" style="display: none;">
        ...
        <h3 class="text-xl font-bold text-slate-800 mb-2 flex items-center gap-2"><i class="ph-fill ph-file-zip text-indigo-500"></i> Import Quiz Data</h3>
        <p class="text-sm text-slate-500 mb-6 font-medium">Upload a ZIP file containing <code class="bg-slate-100 text-orange-600 px-1 rounded">quiz.csv</code> and all optional question/option images.</p>

        <form action="{{ route('faculty.courses.quizzes.import', [$course->id, $quiz->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            
            <div class="border-2 border-dashed border-slate-300 rounded-xl p-8 mb-6 bg-slate-50 flex flex-col items-center justify-center relative hover:border-indigo-400 hover:bg-orange-50/30 transition-colors cursor-pointer group">
                <i class="ph-fill ph-upload-simple text-4xl text-slate-400 group-hover:text-indigo-400 mb-3 transition-colors"></i>
            ...
            <div class="bg-orange-50/50 border border-indigo-100 rounded-xl p-4 mb-6 flex items-start gap-3">
                <i class="ph-fill ph-info text-indigo-500 text-lg mt-0.5"></i>
                <div class="text-xs text-slate-600">
                    <strong class="text-indigo-800 block mb-1">CSV Template Structure:</strong>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <a href="{{ asset('quiz_template.csv') }}" download="quiz_template.csv" class="text-xs font-bold text-orange-600 hover:text-indigo-800 flex items-center gap-1"><i class="ph-bold ph-download-simple"></i> Download Template CSV</a>
                <div class="flex justify-end gap-3 w-full sm:w-auto">
                    ...
                    <button type="submit" class="btn-primary text-white font-bold py-2.5 px-8 rounded-xl shadow-md shadow-orange-500/20 transition-all text-sm flex items-center justify-center gap-2 w-full sm:w-auto"><i class="ph-bold ph-upload-simple"></i> Run Importer</button>
                </div>
            </div>
        </form>
    </div>
